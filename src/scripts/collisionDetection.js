...
function checkCollision(character, obstacles, coins, aliens, missile) {
  if (
    isCollision(character, missile, platform) &&
    SHOULD_COLLIDE_WITH_OBSTACLE_AND_MISSILE
  ) {
  ...
  obstacles.forEach((pair) => {
    ...
    if (
      (isCollision(character, obstacle1) ||
        isCollision(character, obstacle2)) &&
      SHOULD_COLLIDE_WITH_OBSTACLE_AND_MISSILE
    ) {
  ...
  coins.forEach((coin, index) => {
    if (isCollision(character, coin)) {
      ...
      let coinAudio = new Audio(
        "https://www.superluigibros.com/downloads/sounds/SNES/SMK/wav/coin.wav"
      );
      coinAudio.play();
